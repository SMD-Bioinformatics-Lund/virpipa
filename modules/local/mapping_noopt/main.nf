process MAP_READS_NOOPT {
    ...
    cpus { (params.use_sentieon instanceof Boolean ? params.use_sentieon : params.use_sentieon?.toString()?.toBoolean()) ? 16 : 8 }
    memory { (params.use_sentieon instanceof Boolean ? params.use_sentieon : params.use_sentieon?.toString()?.toBoolean()) ? '32 GB' : '8 GB' }
    time '8h'
