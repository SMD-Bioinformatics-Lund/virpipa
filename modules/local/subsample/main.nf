process SUBSAMPLE_READS {
    ...
    if (container_dir) {
        ...
        ${seqtk} sample -s100 ${read1} ${nreads} | gzip -c > ${sample_id}_R1_001.fastq.gz
        ${r2_arg ? "${seqtk} sample -s100 ${read2} ${nreads} | gzip -c > ${sample_id}_R2_001.fastq.gz" : "touch ${sample_id}_R2_001.fastq.gz"}
